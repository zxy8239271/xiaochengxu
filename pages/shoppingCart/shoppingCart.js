...
Page({
  ...
  onLoad: function() {
    ...
    console.log(app.loginInfoData.scope == 'company' && app.loginInfoData.store_id == "0" ? true : false)
    this.setData({
      showdel: false,
      ...
      if (res.errorNo == '0') {
        ...
        _this.data.shopData.filter(function(item, index) { //遍历json对象的每个key/value对,p为key
          ...
          item.son.filter(function(val, i) {
            ...
            // } else {
            price = price + val.supply_price * val.nums;
  ...
  shopJian: function(e) { //减号
    ...
    arr[index].price = arr[index].price - 1 * value.supply_price
    ...
    this.data.allPrice = this.data.allPrice - 1 * value.supply_price
    this.setData({
  ...
  shopJia: function(e) { //加号
    if (util.authFind('shopcar_update')) { //检验权限
      ...
      this.data.allPrice = this.data.allPrice + value.supply_price * 1
      // 数量的加处理
      this.setData({
        allPrice: this.data.allPrice
      });
  ...
  changeShop: function(e) { //手动修改数量
    ...
    this.data.allPrice = this.data.allPrice + value.supply_price * parseFloat(util.toFix(newNum) - value.nums)
    arr[index].price = arr[index].price + value.supply_price * parseFloat(util.toFix(newNum) - value.nums);
    ...
    arrList.filter(function(value, index) { //所有产品去分一下供应商
      arr1.filter(function(item, m) {
        if (value.supplier_id == item.supplier_id) {
          ...
          arr1[m].allPrice += value.supply_price * value.nums - 0;
          arr1[m].allNum += 1;
        }
      })
    })
        ...
        arrClass.filter(function(val, n) { //所有供应商下的产品去做分类
          if (val.pid == value.pid) {
            if (item.classifly.length > 0) {
              var res = item.classifly.findIndex(function(v, i) {
                return v.pid == value.pid
              })
              if (res != '-1') {
                console.log(3333)
                item.classifly[res].kindPrice = item.classifly[res].kindPrice + value.supply_price * value.nums;
                ...
                item.classifly.push({
                  ...
                  pid: val.pid,
                  kindNum: 1,
                  kindPrice: value.supply_price * value.nums,
                  carDetailIsShow: false,
                  children: new  Array(value)
                })
                // console.log(item.classifly)
                // item.classifly[m].kindPrice = item.classifly[m].kindPrice + value.supply_price * value.nums;
  ...
  changeSelect: function(e) { //更改供应商
    var _this = this;
    var oneprice = e.currentTarget.dataset.item.supply_price;
    ...
    this.data.supplierData.filter(function(item, i) {
      if (dataOne.id == item.id) {
        ...
        // _this.data.oneData.supply_price = item.supply_price;
      } else {
    ...
    this.data.shopData[j].price = this.data.shopData[j].price + (oneprice - this.data.oneData.supply_price) * this.data.oneData.nums;
    this.data.allPrice = this.data.allPrice + (oneprice - this.data.oneData.supply_price) * this.data.oneData.nums
    this.data.oneData.supply_price = oneprice;
