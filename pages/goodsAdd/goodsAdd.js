...
Page({
  ...
  onLoad: function(options) {
    ...
    if (classData && classData.length > 0) {
      ...
      console.log("请求分类");
      app.netWork.postJson(app.urlConfig.productinfoPlevelUrl, {
        level: 0,
        type: 0
      }).then(res => {
        if (res.errorNo == 0) {
  ...
  //   });
  // },
  changeGoodsName: function(e) { // 输入、修改商品名称时触发
    this.data.goods.goodsName = e.detail.value.trim();
  },
  changeSpec: function(e) {
    this.data.goods.spec = e.detail.value;
  },
  changeAddress: function(e) { // 详细地址编辑时触发
    this.data.goods.address = e.detail.value;
  },
  changeDescription: function(e) { // 备注输入时触发
    this.data.goods.description = e.detail.value.trim();
  },
  changeIsDown: function(e) { // 改变商品上下架的状态
  console.log(e)
    if (e.detail.value) {
   
      this.data.goods.is_down=0;
    } else {
   
      this.data.goods.is_down = 1;
    }
  

  },
  saveGoods: function() { //新增商品
    var _this = this;
    if (this.data.goods.goodsName == "") { // 做判断,商品名称 详细地址 
      wx.showToast({
        title: '请输入商品名称',
    ...
    var data = {
      ...
      spec: this.data.goods.spec,
      address: this.data.goods.address,
      provinces: this.data.regionResult[0].region_id,
      cities: this.data.regionResult[1].region_id,
      county: this.data.regionResult[2].region_id,
